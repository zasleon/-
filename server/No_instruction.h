...
//1  战士   血5  攻3
//2  法士   血5  攻3
void set_character(int choice,player* p1)//此函数对传入的玩家p的能力初始化为编号为choice的能力
{
...
char* search_character(int number)//查询编号代表的角色名称或人物效果
{
...
//---------------------------------------------------效果卡---------------------------------------------------
	...
	catalog[400][0]="连攻";
	catalog[400][1]="【自己普攻成功时或自己回合自己未处于搏斗状态可发动】自己发动一次普攻\n";
...
char* search_skill(int number)//查询编号代表的武器/卡片效果
{
...
int get_hideeffect(int number)//根据卡片获取其隐效果
{
	...
	cout<<"奇怪的number="<<number<<endl;
	return attack_plus;//连攻
}
